...
/*
 * This is an  improvement on London-based vacation cost calculator program.
 * There are three different types of London-based vacations, the cost for each
 * one is calculated by a single method that is customized based on an input
 * parameter for each vacation. The common portions are inserted directly into
 * the method. The custom portions are handled by separate conditional statements
 * based on the input parameter to the method.
 * 
 ...
 * 
 */
public class VacationCalculatorCombinedMethods
{
	...
	public int costOfEuropeanVacation(int vacationOption)
	{
		...
		if (vacationOption == 1) { //Option 1: Week in London
		...
		else if (vacationOption == 2) { // Option 2: European cities
		...
		else {  // Option 3: bicycling in England
		...
		if (vacationOption == 1) {
		...
		else if (vacationOption == 2) {  // Option 2: lodging in other cities
			...
			System.out.println("Six nights of campground fees: $120");
	...
	{
		VacationCalculatorCombinedMethods vc2 = new VacationCalculatorCombinedMethods();
		System.out.println("Comparison of prices of three London-based vacation packages:\n");
		System.out.println("##################################################################\n");
		System.out.println("Total cost: $"+vc2.costOfEuropeanVacation(1));
		System.out.println("##################################################################\n");
		System.out.println("Total cost: $"+vc2.costOfEuropeanVacation(2));
		System.out.println("##################################################################\n");
		System.out.println("Total cost: $"+vc2.costOfEuropeanVacation(3));
		System.out.println("##################################################################\n");
	}
}
